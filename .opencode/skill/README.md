...
Skills are reusable prompts that teach OpenCode specific patterns for Java development.
...
|------|---------|----------|
| `SKILL.md` | Instructions for OpenCode | AI (loaded via the `skill` tool) |
...
1. Create folder: `.opencode/skill/<skill-name>/`
2. Create `SKILL.md` with instructions for OpenCode
...
```bash
# Automatic - OpenCode detects when to use skills
> "Commit these changes"        # Loads git-commit
> "Review this code for SOLID"  # Loads solid-principles

# Manual - ask OpenCode to use a specific skill
> "Use the git-commit skill to draft a commit message"
```

## Learn More

- [OpenCode Skills Documentation](https://opencode.ai/docs/skills) - Official guide on creating and using skills
